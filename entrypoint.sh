...
if [ ! -f "/app/static/default_image.jpg" ]; then
    ...
    DOWNLOAD_URL=$(curl -s "https://cloud-api.yandex.net/v1/disk/public/resources/download?public_key=${DEFAULT_IMAGE_URL:-https://disk.yandex.ru/i/FdjYbPMKVSBJZQ}" | grep -o '"href":"[^"]*"' | cut -d '"' -f 4)
